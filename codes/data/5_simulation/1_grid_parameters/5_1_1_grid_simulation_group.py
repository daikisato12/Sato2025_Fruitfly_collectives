...
# python3 5_1_1_grid_simulation_group.py 100 0 0.01 0
...
log_param_t = float(args[2])
log_param_a = float(args[3])     
log_param_b = 1 - log_param_t * np.array(log_param_a)

freezing_rate = 0.9              # freezing rate #0.25 #0.4 #0.6 #0.7 #0.8 #0.75 #0.9 #1.0 #0.9

social_influence = float(args[4]) # Coefficient of aligning speed with neighboring flies
...
f = lambda x: x ** 3
repn = int(args[1])
outdir = f"../data/5_simulation/1_grid_parameters/raw/grid_simulation_group_{repn}_{log_param_t}_{log_param_a}_{social_influence}"
